...
class TGraph:
    ...
    def get_file(self , index):
        file = os.path.join(self.stats_directory , 'repositories.csv')
        return file

    def get_dataframe(self):
        repositories = pd.read_csv(self.file , encoding='utf-8')
    ...
    def get_languange_df(self):
        languages = self.repositories['Programming Language'].apply(lambda row: row.replace('[', '').replace(']', '').replace('\'', '').split(',') if not self.isZero(row) else ['None'])
