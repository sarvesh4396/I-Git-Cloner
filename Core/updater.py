import os
import re
from Utils.colours import run, que, good, green, end, info
...
def updater():
    ...
    if changes not in latest_commit:
        ...
        print('%s A new version of I-Git Cloner is available.' % good)
        ...
        if choice != 'n':
            print('%s Updating I-Git Cloner' % run)
            os.system('git clone --quiet https://github.com/s0md3v/Photon %s'
                      % (folder))
            os.system('cp -r %s/%s/* %s && rm -r %s/%s/ 2>/dev/null'
                      % (path, folder, path, path, folder))
            print('%s Update successfull!' % good)
    else:
        print('%s I-Git Cloner is up to date!' % good)
